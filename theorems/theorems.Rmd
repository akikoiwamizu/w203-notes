...
* $E[\epsilon] = 0$. This first moment condition is really about the vertical positioning, is locking us into this one point. 
* If $g$ is a function of $X$, then $Cov[g(X), \epsilon] = 0$. This second moment condition is about the rotation, it's about the angle of the line.
* $E[\epsilon | X] = 0$.
* $V[\epsilon | X] = V[Y | X]$.
* $V[\epsilon] = E[V[Y | X]]$.
:::
...
It can be shown that $E[Z] = 0$ and $V[Z] = \sigma[Z] = 1.$ These convenient properties are the reason why $Z$ is called the standardized sample mean.
:::


::: {.theorem name="Central Limit Theorem (CLT)"}
Let $X_1, X_2, ..., X_n$ be i.i.d. random variables with finite $E[X] = \mu$ finite variance $V[X] = \sigma^2 > 0$, let $Z$ be the standardized sample mean. Then,

$$Z \to^d N(0,1),$$
or equivalently,

$$\sqrt{n}(\bar{X} - \mu) \to^d N(0,\sigma^2).$$
The Central Limit Theorem says that, if $n$ is large, the sampling distribution of the sample mean will tend to be approximately normal even when the population under consideration is not distributed normally. This is useful because it will allow us to rigorously quantify our uncertainty when $n$ is large. 
:::


::: {.theorem name="Slutsky's Theorem"}
Let ($S_{(1)}, S_{(2)}, S_{(3)}...$) and ($T_{(1)}, T_{(2)}, T_{(3)}...$) be sequences of random variables. Let $T$ be a random variable and $c \in \mathbb{R}$. If $S_{(n)} \to^p c$ and $T_{(n)} \to^d T$, then 

* $S_{(n)} + T_{(n)} \to^d c + T.$
* $S_{(n)}T_{(n)} \to^d cT.$
* $\frac{T_{(n)}}{S_{(n)}} \to^d \frac{T}{c}$, provided that $c \neq 0.$

This theorem can be derived from a variant of the Continuous Mapping Theorem. We will often invoke this theorem for the special case in which $S_{(n)}$ is a degenerate random variable, either depending solely on $n$ (for example, $\frac{n}{n-1} \to^p 1$) or just a constant (for example, $c \to^p c$). As the sample size increases, the distribution of the sample mean becomes both bell-shaped and increasingly tightly centered on the population mean. This holds regardless of the particular underlying distribution of $X$, as long as it has finite variance.
:::
...
::: {.definition name="Empirical CDF"}
In general, the CDF of a random variable tells us everything about the behavior of the random variable. The CDF of $X$ is

$$F(x) = Pr[X \leq x] = E[I(X \leq x)].$$
Accordingly, there is a sample analog to the CDF, called the empirical CDF. For i.i.d. random variables $X_1, X_2, ..., X_n$, the empirical CDF $\hat{F}$ of $X$ is

$$\hat{F}(x) = \overline{I(X \leq x)}, \forall x \in \mathbb{R}.$$
Just as the CDF is a function that fully describes the population, the empirical CDF is a function that fully describes the sample. Note that the empirical CDF is always a nondecreasing step function, like the CDF of a discrete random variable. There are two (equivalent) ways of thinking about how $\hat{F}(x)$ is computed:

* Replace the expected value in the definition of the CDF with the sample mean.
* Count the number of observations $i$ satisfying $X_i \leq x$ and divide by $n$.

We know, because of the properties of sample mean, that, $\forall x \in \mathbb{R}, \hat{F}(x)$ will be unbiased and consistent for $F(x)$. Let's take a moment to establish some necessary notation. Let $F$ be the CDF of a random variable $X$ and let $f$ be the corresponding PMF/PDF. Let $g$ be any function of $X$. Then let

$$\int_{-\infty}^{\infty}g(x)dF(x) = \begin{cases} \sum_xg(x)f(x) &: F, discrete \\ \int_{-\infty}^{\infty}g(x)f(x)dx &: F, continuous. \end{cases} $$
:::


::: {.definition name="Statistical Function"}
A statistical function is an operator on a random variable that is represented by a function of the CDF. It is simply a way to represent a feature of a random variable. A statistical function is a function $T: F \to R$, where $F$ is the set of all possible CDFs. For example, for the random variable $X$ with CDF $F$, we can write the expected value of X as:

$$E[X] = T_E(F) = \int_{-\infty}^{\infty}xdF(x).$$
Likewise, we can write the variance of $X$ as:

$$\begin{align} V[X] &= T_V(F) \\ &= \int_{-\infty}^{\infty}(x-E[X])^2dF(x) \\ &= \int_{-\infty}^{\infty}x^2dF(x) - E[X]^2 \\ &= \int_{-\infty}^{\infty} x^2dF(x) + (\int_{-\infty}^{\infty}xdF(x))^2. \end{align}$$
:::


::: {.definition name="Plug-In Estimator"}
For i.i.d. random variables $X_1, X_2, ..., X_n$, with common CDF $F$, the plug-in estimator of $\theta = T(F)$ is:

$$\hat{\theta} = T(\hat{F}).$$
To estimate $\theta = T(F)$, apply the same function to the empirical CDF. Thus,

$$\hat{E}[X] = \frac{1}{n}\sum_{i=1}^nx = \bar{X},$$
$$\hat{V}[X] = T_V(\hat{F}) = \overline{X^2} - (\bar{X})^2.$$
:::


# Hypothesis Testing

### Devore & Berk - Section 9.1
...
* However, the objective of an investigation is not to estimate the parameter, but to decide which of two contradictory claims about the parameter is correct. Methods for accomplishing this comprise the part of statistical inference called **hypothesis testing**.
* A **statistical hypothesis** is a claim or assertion either about the value of a single parameter (i.e. a characteristic of a population or a probability distribution), about the values of several parameters, or about the form of an entire probability distribution. 
* The **null hypothesis** (H_0) is the claim that is initially assumed to be true.
* The **alternative hypothesis** (H_a) is the assertion that is contradictory to H_0
* The two plausible conclusions from a hypothesis-testing analysis are 1) reject H_0 or 2) fail to reject H_0
* A **test statistic** is a function of the sample data used to make a decision
* A **rejection region** is a region consisting of those test statistic values for which H_0 will be rejected in favor of H_a.
* **Type 1 error** consists of rejecting the null hypothesis when it is true - a type 1 error is often called a **false positive**.
* **Type 2 error** involves not rejecting the null hypothesis when it is false (i.e. H_a is true) - a type 2 error is often called an **opportunity loss** in business.
  * Example: Since in the US judicial system the null hypothesis is that the accused is innocent, a type 1 error is the same as convicting an innocent person, while a false acquittal (i.e. letting a guilty person go free) equates to a type 2 error.
* **Proposition**: Once the test statistic and n are fixed, there is no rejection region that will simultaneously make both type 1 and type 2 errors small.
  * The approach that most people take is to specify the largest value of type 1 error that can be tolerated and find a rejection region having that value. This makes the type 2 error as small as possible subject to the bound of the type 1 error. The resulting value of the type 1 error is often referred to as the **significance level** of the test.
  * A test with a significance level alpha is one for which the type 1 error probability is controlled at the specified level.
* **Power** is defined as the probability that  the test procedure will fail to reject the null hypothesis. Most software packages calculate type 2 error probabilities for a variety of test procedures in terms of power.


### Devore & Berk - Section 9.2
* Recall that the Z Statistic obtained by standardizing $\bar{X}$ under the assumption that $H_0$ is true:

$$Z = \frac{\bar{X} - \mu}{\frac{\sigma}{\sqrt{n}}}.$$

As discussed in the previous section, the cutoff value $c$ should be chosen to control the probability of a type I error at the desired level $\alpha$. This is easily accomplished because the distribution of the test statistic $Z$ when $H_0$ is true is the standard normal distrbution:

$\alpha$ = P(type I error) = P($H_0$ is rejected when $H_0$ is true)
$\alpha$ = P($Z \geq c$ when $Z \sim N(0,1)$) = 1 - $\Phi(c) \Rightarrow$ 
$\Phi$ = 1 - $\alpha \Rightarrow c = \Phi^{-1}(1 - \alpha) = z_{\alpha}.$ 

Thus, the rejection region $z \geq z_{\alpha}$ has type I error probability $\alpha$.

* We can now modify the one-sample z-test to accommodate the more realistic situation when $\sigma$ is unknown. 

$$T = \frac{\bar{X} - \mu_0}{\frac{S}{\sqrt{n}}}.$$ 

::: {.definition name="Noncentral t Distribution"}
Let $Z \sim N(0,1)$ and $Y \sim \chi_v^2$ be independent random variables. For any real number $\delta$, the random variable

$$\frac{Z + \delta}{\sqrt{\frac{Y}{v}}}$$
has a non-central t-distribution with $v$ degrees of freedom and noncentrality parameter $\delta$.
:::

* For any fixed difference $\mu' - \mu_0$, power increases with sample size. That is, for any fixed departure from $H_0$, a larger sample size will increase the likelihood of correctly detecting that $H_0$ is false and $H_a$ is true.
* For any fixed sample size, power increases as the "difference" increases (i.e. the distance between $\mu'$ and $\mu_0$ grows.)


### Devore & Berk - Section 9.4
...
* Examples:
  * P-value = 0.250 means that 25% of all possible test statistic values are more contradictory to $H_0$ than the one that came out of our sample. So our data is not all that contradictory to the null hypothesis.
  * P-value = 0.0019 means that 0.19& of all possible test statistic values are at least as contradictory to $H_0$ as what we obtained. Thus, the sample appears to be highly contradictory to the null hypothesis.
...
* **Decision Rule Based on the P-Value**: select a significance level $\alpha$ (aka the type I error). Then reject $H_0$ if the p-value $\leq \alpha$; do not reject $H_0$ if the p-value is $> \alpha$.